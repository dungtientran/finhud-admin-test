...
function Layout({ children }) {
  ...
  useEffect(() => {
    ...
    if (user && !loading) {
      setToken();
    } else if (router.pathname !== "/recover-password") {
      router.push("/login");
    }
    ...
    <>
      {loading && (
        <div className="loading wr">
          <p>
            <LoadingOutlined
              style={{ fontSize: "50px", color: "var(--orange)" }}
            />
          </p>
        </div>
      )}

      {user && router.pathname !== "/recover-password" && (
      ...
      {router.pathname === "/recover-password" && <div>{children}</div>}
    </>
  );
}
